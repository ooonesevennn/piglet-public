...
class MixedAgent(CaptureAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        CURRENT_ACTION[self.index]=None
        """
    ...
    #------------------------------- PDDL Functions ------------------------------- 
    def get_pddl_state(self,gameState:GameState) -> List[Tuple]:
        # Collect objects and states from the gameState

        states = []
        objects = []

        # TODO: collect is_scared
        ...
        agents = [(i,gameState.getAgentState(i)) for i in self.getTeam(gameState)]
    ...
    def stateSatisfyCurrentPlan(self, init_state):
        ...
        if self.pddl_solver.satisfyPrecondition(init_state, self.plan[self.currentActionIndex][0]):
            # Current action precondition still satisfied, continue executing current action of the plan
            return True
        
        # Current action precondition not satisfied anymore, need new plan
        return False
    
    def goalScoring(self,objects, initState):
        positiveGoal = []
        ...
        for obj in objects:
            ...
            if agent_type == "current_agent":
                negtiveGoal += [("1_food_in_backpack", agent_obj)]
    ...
    def getGoals(self, objects, initState):
        # TODO: Goal prioritization and selection.
        positiveGoal = []
        negtiveGoal = [("food_available",)]
