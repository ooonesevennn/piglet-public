...
from lib_piglet.search.iteritive_deepening import iterative_deeping_astar, iterative_deeping_dfs
...
search = iterative_deeping_astar(stack(), expander, time_limit=10, heuristic_function=n_puzzle_h.sum_manhattan_heuristic)
path = search.get_path(puzzle.start_state(), puzzle.goal_state())
print(statistic_template.format("","",*[str(x) for x in search.get_statistic()], str(search.solution_)))

search = iterative_deeping_dfs(stack(), expander, time_limit=10, heuristic_function=None)
path = search.get_path(puzzle.start_state(), puzzle.goal_state(),5)
print(statistic_template.format("","",*[str(x) for x in search.get_statistic()], str(search.solution_)))

search = iterative_deeping_dfs(stack(), expander, time_limit=10, heuristic_function=None)
path = search.get_path(puzzle.start_state(), puzzle.goal_state(),20)
print(statistic_template.format("","",*[str(x) for x in search.get_statistic()], str(search.solution_)))
