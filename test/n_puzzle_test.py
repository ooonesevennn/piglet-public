...
search = graph_search(bin_heap(compare_node_f), expander, time_limit=10, heuristic_function=n_puzzle.n_puzzle_manhattan_heuristic)
path = search.get_path(puzzle.start_state(), puzzle.goal_state())
print(statistic_template.format("","",*[str(x) for x in search.get_statistic()], str(search.solution_)))

search = graph_search(bin_heap(compare_node_f), expander, time_limit=10, heuristic_function=n_puzzle.n_puzzle_straight_heuristic)
path = search.get_path(puzzle.start_state(), puzzle.goal_state())
print(statistic_template.format("","",*[str(x) for x in search.get_statistic()], str(search.solution_)))
